/*
 * itemContext.js — resolve the current Zotero item's PDF and build a per-item
...
(function (root) {
  ...
  async function resolvePdfAttachment(item) {
    if (!item) return null;
  ...
  async function prepareWorkdir(item) {
  ...
  root.PRAItemContext = { prepareWorkdir: prepareWorkdir, resolvePdfAttachment: resolvePdfAttachment };
