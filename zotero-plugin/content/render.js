/*
 * render.js — render assistant Markdown + LaTeX math into SAFE HTML.
 *
 * Pipeline (the markdown/math + sanitization ordering matters):
 *   1. pull out math spans ($$..$$, \[..\], \(..\), $..$) → placeholder tokens
 *      (so markdown can't mangle TeX like x_i, and TeX can't break markdown)
 ...
 *
 * Returns an HTML string, or null when the libs are missing / anything fails —
 * callers MUST fall back to plain textContent on null (never inject raw HTML).
 *
 * Depends on globals bundled in content/vendor/: marked, DOMPurify, katex.
 * No Zotero/Gecko APIs here except the optional DOMPurify window, so it is a
 * pure module (and unit-testable under Node with the same globals).
 */
(function (root) {
  "use strict";
  ...
  function getPurify(win) {
    ...
    var D = root.DOMPurify;
    if (!D) return null;
    // Loaded without a window, DOMPurify is a factory; call it with a window.
    _purify = (typeof D.sanitize === "function") ? D : (typeof D === "function" ? D(win) : null);
    return _purify;
  }
  ...
  function render(text, win) {
    var marked = root.marked, katex = root.katex;
