/*
 * chatPanel.js — the Zotero item-pane chat section (privileged chrome only).
 *
...
(function (root) {
  ...
  function setRich(bubble, text, attachmentID) {
    ...
    try {
      ...
      if (root.PRARender && win) html = root.PRARender.render(text, win);
    } catch (e) { html = null; }
  ...
  function startTurn(session, content) {
    ...
    root.PRAChatService.runTurn(prefs(), session.ctx, session.conv, content, adapter, run.liveRef)
  ...
  async function mount(body, item) {
    ...
    root.PRACodexDriver.healthcheck(prefs()).then(function (h) {
    ...
    var ctx;
    try {
      ctx = await root.PRAItemContext.prepareWorkdir(item);
    } catch (e) {
    ...
    session.ctx = ctx;
    if (!session.conv) session.conv = await root.PRAStore.load(ctx.key);
  ...
  function register(arg) {
    ...
    try {
      ...
      // the plugin's FTL (locale/en-US/paper-reading-agent.ftl), injected into
      // each window in bootstrap.js. Icons are background-images themed via
      // -moz-context-properties, so the SVGs use fill="context-fill".
      SECTION_ID = Zotero.ItemPaneManager.registerSection({
        ...
        onDestroy: function () { /* keep any running turn alive across re-render; the next mount re-attaches. Runs are killed only by Stop or plugin shutdown (PRAAppServer.shutdown). */ },
      });
  ...
  function dump() {   // debug: inspect per-paper session state from Run JavaScript
    var out = {};
    try {
  ...
  root.PRAChatPanel = { register: register, unregister: unregister, dump: dump };
  try { Zotero.PaperReadingAgent = root.PRAChatPanel; } catch (e) {}   // reachable from Run JavaScript for debugging
