/*
 * codexDriver.js — the ONLY codex-aware module. Spawns `codex exec` /
 * `codex exec resume` via Mozilla's Subprocess (privileged chrome / Zotero only)
 * and streams AgentEvents through PRAEventMapper.
 *
 * `buildArgv` is pure (no Gecko APIs) and is unit-tested under Node. Everything
 * else touches Subprocess only at call time, so this file still loads in Node.
 *
...
(function (root) {
  ...
  function buildArgv(workdir, sessionId, prompt, opts) {
    opts = opts || {};
  ...
  async function runTurn(opts, workdir, sessionId, prompt, onEvent, liveRef) {
    opts = opts || {};
    ...
    try {
      ...
      while ((chunk = await proc.stdout.readString())) {
        var r = root.PRAEventMapper.consume(buffer, chunk);
        buffer = r.buffer;
  ...
  var api = { buildArgv: buildArgv, runTurn: runTurn, healthcheck: healthcheck };
  root.PRACodexDriver = api;
