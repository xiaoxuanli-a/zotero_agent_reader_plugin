/*
 * eventMapper.js — pure mapping of codex `--json` JSONL events to the
 * backend-neutral AgentEvent vocabulary. NO Zotero/Gecko APIs, so it is
 * unit-testable under plain Node AND loadable in Zotero (assigns a global).
 *
...
(function (root) {
  ...
  var api = { mapEvent: mapEvent, consume: consume, extractText: extractText };
  root.PRAEventMapper = api;
