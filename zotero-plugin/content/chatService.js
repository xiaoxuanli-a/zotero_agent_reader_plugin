/*
 * chatService.js — orchestrate one chat turn (Zotero / privileged chrome only).
 *
 * Mirrors app/chat_service.py: persist the user message, append an empty
 * assistant message up front, drive the codex turn, accumulate deltas, and
 * persist on done AND on error/teardown so partial answers survive.
 *
...
(function (root) {
  ...
  async function runTurn(opts, ctx, conv, content, ui, liveRef) {
    var userMsg = { role: "user", content: content };
    var assistant = { role: "assistant", content: "" };
    conv.messages.push(userMsg);
    conv.messages.push(assistant);
    await root.PRAStore.save(conv);
    ...
    function persist() { root.PRAStore.save(conv); }

    await root.PRAAppServer.runTurn(opts, ctx.workdir, conv.codex_session_id, content, function (ev) {
      if (ev.kind === "thread_started") {
        if (ev.sessionId && !conv.codex_session_id) {
          conv.codex_session_id = ev.sessionId;
          root.PRAStore.save(conv);
        }
  ...
  root.PRAChatService = { runTurn: runTurn };
