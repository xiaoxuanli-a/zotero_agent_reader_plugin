...
export async function runTurn(opts, workdir, threadId, prompt, onEvent, liveRef) {
  opts = opts || {};
  ...
  try {
    ...
    // web_search is a process-level flag set at app-server start, so we can't drop it
    // per turn — bump minimal → low (the closest still-fast effort) when search is on.
    if (effort === "minimal" && opts.webSearch !== false) {
      log("effort 'minimal' is incompatible with web_search; using 'low' for this turn");
      effort = "low";
    }
